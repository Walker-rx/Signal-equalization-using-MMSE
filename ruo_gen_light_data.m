...
%%
t = datetime('now');
save_path = "vol_save/dnn_data/"+t.Year+"."+t.Month+"."+t.Day+"/"+origin_rate_tmp/1e6+"M"+"/"+M+"pam";
...
%         unmod = [pilot,zeros(1,zero_length),data];
  %%      
        data_mpam_ini = randn(1,data_length);
        data_mpam_tmp = conv(data_mpam_ini,filter_10m);
...
%             fprintf('\n');
            fprintf("amp = %d , snr = %f , save time =%d \n",amp,snr_ls_beforecorrect,save_time);
        end
    end
    snr = snr/save_num;
    if amp == amp_begin
        save_snr = fopen(save_path+"/save_errnum.txt",'w');
    else
        save_snr = fopen(save_path+"/save_errnum.txt",'a');
    end
    fprintf(save_snr,'%f \n',snr);
    fclose(save_snr);
    fprintf('amp = %d , snr = %f \n',amp,snr);
