...
for amp = amp_begin:amp_end
    ...
    while(errornum_ls <= 30 || looptime < 2000)
        ...
        pn_smi = bandpower(noise)*8192^2;
        ps_smi = pn_smi*10^(snr/10);
        
        channel_smi = ruo_channel_coefficient(signal_ori,signal_received,times,coar_syn_point,pilot_length,num_of_windows);
        channel_smi = ruo_sam_rate_con(channel_smi,filter_transmit,upf_transmit,dof_transmit);
        channel_smi = channel_smi./norm(channel_smi,2)*sqrt(length(channel_smi));
        channel_delay_smi = length(channel_smi);
        
        signal_send_smi = signal_upsample;
        signal_send_smi = signal_send_smi./norm(signal_send_smi,2)*sqrt(length(signal_send_smi))*sqrt(ps_smi);
        
        signal_passtap_smi = conv(signal_send_smi,channel_smi);
        signal_passchannel_smi = signal_passtap_smi((channel_delay_smi+1)/2:length(signal_passtap_smi)-(channel_delay_smi-1)/2);
        signal_passchannel_smi = signal_passchannel_smi./norm(signal_passchannel_smi,2)*sqrt(length(signal_passchannel_smi))*sqrt(ps_smi);
        noise_smi = sqrt(pn_smi)*randn(1,length(signal_passchannel_smi));
        signal_passchannel_smi = signal_passchannel_smi + noise_smi;
...
%     ser_ls = gather(ser_ls);
    ...
    if amp == amp_begin
        fsnr = fopen(save_path+"/snr_smi.txt",'w');
        fser = fopen(save_path+"/ser_smi.txt",'w');
        ...
        fsnr = fopen(save_path+"/snr_smi.txt",'a');
        fser = fopen(save_path+"/ser_smi.txt",'a');
    end
    fprintf(fsnr,'%.8f \r\n',snr);
    fprintf(fser,'%.6g \r\n',ser_ls);
    fclose(fsnr);
